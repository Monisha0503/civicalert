...
const fetchIssues = async () => {
    try {
      const res = await axios.get("https://civicalert-backend-bdgdb7h2aqbfdjgk.centralindia-01.azurewebsites.net/api/issues/all", {
  ...
  const updateStatus = async (id, status) => {
    try {
      await axios.put(`https://civicalert-backend-bdgdb7h2aqbfdjgk.centralindia-01.azurewebsites.net/api/issues/status/${id}`, { status }, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchIssues();
    } catch (err) { console.log(err); }
